...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Shared expense status updated successfully' }, { status: 200 });
  } catch (error) {
    console.error('Error updating shared expense status:', error);
    return NextResponse.json({ error: 'Failed to update shared expense status' }, { status: 500 });
  }
}
